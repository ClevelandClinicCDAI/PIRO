...
export class QueryBuilderComponent implements OnInit, OnChanges, ControlValueAccessor, Validator {
  ...
  getDefaultField(entity: Entity) {
    if (!entity) {
      return null;
    } else if (entity.defaultField !== undefined) {
      return this.getDefaultValue(entity.defaultField);
    } else {
      const entityFields = this.fields.filter((field: any) => {
  ...
  addRule(parent?: RuleSet): void {
    ...
    if (this.config.addRule) {
      ...
      //const field = {"field":"final", "value": "", "defaultValue": "", "operator":"contains"};
      const fieldDefault =  this.fields.filter((field:any) => {
        return field && field.value === "final";
      });
      const field = fieldDefault.length == 0 ? this.fields[0] : fieldDefault[0];
  ...
  removeRule(rule: Rule, parent?: RuleSet): void {
    ...
    if (this.config.removeRule) {
      ...
      parent.rules = parent.rules.filter((r: any) => r !== rule);
    }
  ...
  removeRuleSet(ruleset?: RuleSet, parent?: RuleSet): void {
    ...
    if (this.config.removeRuleSet) {
      ...
      parent.rules = parent.rules.filter((r: any) => r !== ruleset);
    }
  ...
  coerceValueForOperator(operator: string, value: any, rule: Rule): any {
    const inputType: any = this.getInputType(rule.field, operator);
  ...
  changeEntity(entityValue: string, rule: any, index: number, data: RuleSet): void {
    if (this.disabled) {
      return;
    }
    let i = index;
    let rs = data;
    const entity: Entity = this.entities.find((e: any) => e.value === entityValue);
    const defaultField: any = this.getDefaultField(entity);
    if (!rs) {
      rs = this.data;
      i = rs.rules.findIndex((x: any) => x === rule);
    }
  ...
  getQueryItemClassName(local: LocalRuleMeta) {
  ...
  getRemoveButtonContext(rule: Rule) {
    ...
    if (ruleset && ruleset.rules && ruleset.rules.length > 0) {
      ruleset.rules.forEach((item: any) => {
        if ((item as RuleSet).rules) {
          ...
          const field:any = this.config.fields[(item as Rule).field];
